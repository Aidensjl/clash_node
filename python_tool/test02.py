#!/usr/bin/env python3
"""
merge_yaml.py — 合并指定目录下所有 .yaml / .yml 文件的 proxies 列表
不包含的目录
--exclude merged.yml backup.yml old.yaml
用法:
    python python_tool/merge_yaml.py MyNode -o MyNode/merged.yml
"""
...
def dedup(nodes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    seen = set()
    out = []
    for n in nodes:
...
def collect_yaml_files(in_dir: Path, output_path: Path, exclude_names: set) -> List[Path]:
    """收集 .yaml 和 .yml 文件，排除输出文件本身及 exclude 名单。"""
    result = []
    seen = set()
...
def main() -> int:
    ...
    ap.add_argument("input_dir", help="包含 .yaml / .yml 的目录")
    ap.add_argument("-o", "--output", required=True, help="输出合并文件")
    ap.add_argument("--dedup", action="store_true", default=True, help="去重（默认开启）")
    ap.add_argument("--no-dedup", dest="dedup", action="store_false")
    ap.add_argument("--exclude", nargs="*", default=[], help="要排除的文件名（如 merged.yml）")
    args = ap.parse_args()
    ...
    if args.dedup:
        ...
        print(f"去重: {before} → {len(all_nodes)}")
